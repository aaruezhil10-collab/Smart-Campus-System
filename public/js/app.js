...
async function navigate(view) {
  state.view = view;
  state.filters = {};
  $('#pageTitle').textContent = TITLES[view] || 'Smart Campus System';
  $('#breadcrumb').textContent = `Smart Campus System / ${state.user.role === 'warden' ? 'Warden' : 'Student'} portal`;
  renderNav();
  closeSidebar();
  skeletonPage();
  try {
...
function showReceipt(fee) {
  modal({title:'Payment receipt',subtitle:'Smart Campus System digital payment record',size:'modal-sm',body:`<div class="text-center"><div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700">${icon('check',25)}</div><h3 class="mt-4 text-lg font-black text-navy-950">Payment successful</h3><p class="mt-1 text-xs text-slate-500">${esc(fee.title)}</p></div><div class="mt-6 space-y-3 rounded-2xl bg-slate-50 p-4 text-xs"><div class="flex justify-between"><span class="text-slate-500">Amount</span><b class="text-navy-950">${money(fee.amount)}</b></div><div class="flex justify-between"><span class="text-slate-500">Paid on</span><b class="text-navy-950">${formatDate(fee.paidAt)}</b></div><div class="flex justify-between gap-4"><span class="text-slate-500">Transaction ID</span><b class="break-all text-right text-navy-950">${esc(fee.transactionId||'—')}</b></div></div><button class="btn-secondary mt-5 w-full" data-action="copy-transaction" data-value="${esc(fee.transactionId||'')}">${icon('copy',16)} Copy transaction ID</button>`});
}
...
async function logout() {
  const okay=await confirmAction({title:'Sign out of Smart Campus System?',message:'Your secure session will end on this device.',confirmText:'Sign out'}); if(!okay)return;
